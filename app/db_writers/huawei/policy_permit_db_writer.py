...
def policy_permit_db_writer(es, index, log_data, iid, qsize):
    doc = {
        'iid': iid,
    ...
    try:
        es.index(index=index, doc_type="policy_permit", body=doc)
    # except elasticsearch.exceptions.ConflictError as ex:
    #     es.delete(index=index, doc_type="policy_permit", id=iid)
    #     es.create(index=index, doc_type="policy_permit", id=iid, body=doc)
    #     raise ex
